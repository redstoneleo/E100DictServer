...
def youdaoEnDict(queryText):  # http://www.iciba.com/teaching%20post  比较全，待测速度；positioned，huayi没有音标，但是iciba搜索有音标；
  ...
  # print('youdaoEnDict---------------', queryText, symbolElement.get_text("★", strip=True))
  if symbolElement:
    try:
...
def youdaoZhToEn(queryText):  # http://www.iciba.com/teaching%20post  比较全，待测速度；positioned，huayi没有音标，但是iciba搜索有音标；
  # queryUrl = 'http://dict-co.iciba.com/api/dictionary.php?w={}&type=json&key=96731B04071B4A35D80AE892279B3D23'.format(queryText)#Hilbert无结果
  queryUrl = 'http://dict.youdao.com/search?q={}'.format(queryText)  # 任务：queryText放到配置文件，翻译innerHTML、空格（前面已经过滤掉了）无结果
  ...
  result = {'dictKeyText':queryText}  # {'ukPhoneticSymbol': '', 'usPhoneticSymbol': ''}  # 如果不加这两项，对于没有音标的返回的是None，None不能写入数据库，所以value这里写 ''
  soup = BeautifulSoup(repliedText, "html.parser")  # 因为是从HTML里获取的HTML，这里.replace('\\', '')是转义
  symbolElementList = soup.select('#phrsListTab .search-js')  # soup.find(class_ = 'in-base')
  # print('youdaoEnDict---------------', queryText, symbolElementList)#.get_text("★", strip=True)
  
  if symbolElementList:
    wordList=[]
    try:
      for symbolElement in symbolElementList:
        word=symbolElement.get_text()
        wordList.append(word)
    except Exception as e:
      print('chineseDefinition-------IndexError--------',queryText,e)
      result.update({'exception':e})
    else:
      result.update({'wordList':wordList})
  return result
